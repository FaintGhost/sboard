...
### Users 状态筛选全量下沉（2026-02-12）
- 已覆盖范围：
  - `active`：DB 侧有效状态分页过滤。
  - `disabled`：DB 侧有效状态分页过滤。
  - `expired`：DB 侧有效状态分页过滤。
  - `traffic_exceeded`：DB 侧有效状态分页过滤。
- 语义保障：
  - 对 `active/traffic_exceeded` 查询，先执行“流量重置归一化”（调用既有 `applyTrafficResetIfNeeded`），再执行 SQL 过滤，避免“到重置日仍被判定超限”。
- 实现细节：
  - 修复了 SQLite 锁冲突风险：先读出候选用户并关闭 rows，再执行逐条重置。
- 未覆盖范围：
