...
export function EditUserDialog({
  ...
  const isPending =
    createMutation.isPending || updateMutation.isPending || saveGroupsMutation.isPending;

  const hasSaveError =
    createMutation.isError || updateMutation.isError || saveGroupsMutation.isError;
  const saveError = createMutation.error ?? updateMutation.error ?? saveGroupsMutation.error;

  return (
    <Dialog open={!!editState} onOpenChange={(open) => (!open ? setEditState(null) : null)}>
      <DialogContent
        aria-label={editState?.mode === "create" ? t("users.createUser") : t("users.editUser")}
      >
        <DialogHeader>
          <DialogTitle>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-1 md:col-span-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setEditState((prev) => (prev ? { ...prev, username: e.target.value } : prev))
                }
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={(v) => {
                            ...
                            setEditState((p) => {
                              ...
                              return {
                                ...p,
                                groupIDs: Array.from(set.values()).sort((a, b) => a - b),
                              };
                            });
                          }}
                        />
            ...
            <>
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setEditState((prev) => (prev ? { ...prev, status: value as UserStatus } : prev))
                  }
                >
              ...
              <div className="space-y-1 md:col-span-2">
                ...
                <div className="flex flex-col gap-2 md:flex-row md:items-center">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setEditState((prev) =>
                        prev ? { ...prev, expireDate: null, clearExpireAt: true } : prev,
                      )
                    }
        ...
        <DialogFooter>
          ...
          <AsyncButton
            onClick={handleSave}
            disabled={!editState || isPending || !editState.username.trim()}
            pending={isPending}
            pendingText={editState?.mode === "create" ? t("common.creating") : t("common.saving")}
          >
