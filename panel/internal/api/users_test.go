...
func TestUsersAPI_CreateListAndDisable(t *testing.T) {
	...
	require.Empty(t, created.Data.GroupIDs)

	group, err := store.CreateGroup(t.Context(), "g1", "")
	require.NoError(t, err)
	require.NoError(t, store.ReplaceUserGroups(t.Context(), created.Data.ID, []int64{group.ID}))
	...
	require.Equal(t, []int64{group.ID}, listed.Data[0].GroupIDs)
