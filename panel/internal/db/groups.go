...
func (s *Store) GetGroupByID(ctx context.Context, id int64) (Group, error) {
  row := s.DB.QueryRowContext(ctx, "SELECT id, name, COALESCE(description, '') FROM groups WHERE id = ?", id)
  var g Group
...
func (s *Store) ListGroups(ctx context.Context, limit, offset int) ([]Group, error) {
  rows, err := s.DB.QueryContext(ctx, "SELECT id, name, COALESCE(description, '') FROM groups ORDER BY id DESC LIMIT ? OFFSET ?", limit, offset)
  if err != nil {
    return nil, err
  }
  defer rows.Close()
